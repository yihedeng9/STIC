...
wandb.login(key="5e90a34f1b00c9eb9687d8e2af651cd85957aa23")
local_rank = None
...
def preprocess(
    ...
    for target, source in zip(targets, sources):
        if has_image:
            tokenized_lens = get_tokenize_len([header] + [s["content"] for s in source])
        else:
            tokenized_lens = _tokenize_fn([header] + [s["content"] for s in source], tokenizer)["input_ids_lens"]
